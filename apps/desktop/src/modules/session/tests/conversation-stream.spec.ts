...
vi.mock("@/modules/session/services", async (importOriginal) => {
  const actual = await importOriginal<typeof import("@/modules/session/services")>();
  return {
    ...actual,
...
vi.mock("@/modules/session/store/executionActions", () => ({
...
describe("conversation stream routing", () => {
  ...
  it("routes mismatched stream events by event.session_id", () => {
    ...
    expect(applyIncomingExecutionEventMock).toHaveBeenCalledWith(
      conversationB.id,
      expect.objectContaining({ session_id: conversationB.id, run_id: "exec_stream_1" })
    );
