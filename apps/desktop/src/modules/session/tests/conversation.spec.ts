...
describe("conversation store", () => {
  ...
  it("keeps composer draft textarea full width via css rule", () => {
    const css = readFileSync(resolve(process.cwd(), "src/modules/session/components/MainConversationPanel.css"), "utf8");
  ...
  it("renders inspector trace details for selected execution", () => {
    ...
    expect(wrapper.text()).toContain("运行: exec_trace_target");
  ...
  it("falls back to latest trace when inspector selected id is empty", () => {
    ...
    expect(wrapper.text()).toContain("运行: exec_trace_latest");
