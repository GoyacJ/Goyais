import { isTerminalRunState } from "@/modules/session/store/executionMerge";
import { normalizeExecutionEventsByExecution } from "@/modules/session/trace/normalize";
import { truncateText } from "@/modules/session/trace/summarize";
import type {
  NormalizedThinkingStage,
  NormalizedTraceEvent,
  OperationIntentKind,
  RunTraceStepViewModel,
  RunTraceViewModelData,
...
export function buildRunTraceViewModelData(
...
export function hydrateRunningActionElapsed(
  ...
  return actions.map((action) => {
    ...
    return {
      ...action,
      elapsedMs,
      elapsedLabel: tr(locale, "session.running.elapsed", { seconds })
    };
  });
}
...
function handleThinkingDeltaEvent(
  ...
  if (event.stage === "run_approval_needed") {
    ...
    activeActions.set(actionId, {
      ...
      toolName,
      comparisonName: "",
      primary: tr(locale, "session.running.primary.approval", { tool: toolName }),
  ...
  if (event.stage === "run_user_question_needed") {
    ...
    activeActions.set(actionId, {
      ...
      comparisonName: "",
      primary: question !== ""
        ? tr(locale, "session.running.primary.userInputQuestion", { question: truncateText(question, 72) })
        : tr(locale, "session.running.primary.userInput"),
      secondary: composeSecondary(locale, latestReasoningSentence, event.operationSummary),
      startedAt: event.timestamp
    });
    return;
  ...
  if (event.stage === "model_call") {
    ...
    activeActions.set(actionId, {
      ...
      primary: modelReasoning !== ""
        ? modelReasoning
        : tr(locale, "session.running.primary.modelAnalyzing"),
      secondary: modelReasoning !== ""
        ? tr(locale, "session.running.secondary.modelReasoning", { reasoning: modelReasoning })
        : tr(locale, "session.running.secondary.modelPending"),
      startedAt: event.timestamp
    });
    return;
  }
...
function buildTraceSummary(
  ...
  if (execution.state === "queued") {
    primary = tr(locale, "session.trace.summary.queued");
  } else if (execution.state === "failed") {
    primary = toolFailedCount > 0
      ? tr(locale, "session.trace.summary.failed.withCount", {
        tools: toolCallCount,
        failed: toolFailedCount
      })
      : tr(locale, "session.trace.summary.failed", { tools: toolCallCount });
  } else if (execution.state === "cancelled") {
    primary = tr(locale, "session.trace.summary.cancelled", { tools: toolCallCount });
  } else if (execution.state === "pending" || execution.state === "executing") {
    primary = thinkingCount > 0
      ? tr(locale, "session.trace.summary.executing.thinking", {
        duration: durationSec,
        tools: toolCallCount
      })
      : tr(locale, "session.trace.summary.executing", {
        duration: durationSec,
        tools: toolCallCount
      });
  } else if (execution.state === "confirming") {
    primary = tr(locale, "session.trace.summary.confirming", {
      duration: durationSec,
      tools: toolCallCount
    });
  } else if (execution.state === "awaiting_input") {
    primary = tr(locale, "session.trace.summary.awaitingInput", {
      duration: durationSec,
      tools: toolCallCount
    });
  } else {
    primary = tr(locale, "session.trace.summary.completed", { tools: toolCallCount });
  }

  const secondary = tokensIn !== null && tokensOut !== null
    ? tr(locale, "session.trace.summary.secondary.withToken", {
      input: tokensIn,
      output: tokensOut,
      total: tokensIn + tokensOut,
      duration: messageDurationSec
    })
    : tr(locale, "session.trace.summary.secondary.noToken", { duration: messageDurationSec });
...
function toTraceStep(
  ...
  if (event.type === "execution_started") {
    return {
      id: stepId,
      kind: "lifecycle",
      title: tr(locale, "session.trace.step.title.lifecycle"),
      summary: tr(locale, "session.trace.step.summary.executionStarted"),
      detail: "",
      timestampLabel,
      statusTone: "neutral",
      rawPayload
  ...
  if (event.type === "thinking_delta") {
    ...
    return {
      id: stepId,
      kind: "reasoning",
      title: tr(locale, "session.trace.step.title.reasoning"),
  ...
  return {
    id: stepId,
    kind: "tool_result",
    title: tr(locale, "session.trace.step.title.toolResult"),
    summary: success
      ? tr(locale, "session.trace.step.summary.toolResultSuccess", { tool: toolName })
      : tr(locale, "session.trace.step.summary.toolResultFailed", { tool: toolName }),
...
function resolveThinkingDetail(locale: TraceLocale, event: NormalizedTraceEvent): string {
  if (event.stage === "run_approval_needed") {
    if (event.operationSummary !== "") {
      return tr(locale, "session.trace.step.detail.operation", { operation: event.operationSummary });
    }
    return tr(locale, "session.trace.step.detail.waitingApproval");
  }

  if (event.stage === "run_user_question_needed") {
    const question = String(event.payload.question ?? "").trim();
    if (question !== "") {
      return tr(locale, "session.trace.step.detail.userQuestion", { question });
    }
    return tr(locale, "session.trace.step.detail.waitingUserInput");
  }
...
function formatThinkingStageLabel(locale: TraceLocale, stage: NormalizedThinkingStage): string {
  switch (stage) {
    case "model_call":
      return tr(locale, "session.trace.stage.modelCall");
    case "assistant_output":
      return tr(locale, "session.trace.stage.assistantOutput");
    case "run_approval_needed":
      return tr(locale, "session.trace.stage.approvalNeeded");
    case "run_user_question_needed":
      return tr(locale, "session.trace.stage.userQuestionNeeded");
    case "run_user_question_resolved":
      return tr(locale, "session.trace.stage.userQuestionResolved");
    case "approval_granted":
      return tr(locale, "session.trace.stage.approvalGranted");
    case "approval_denied":
      return tr(locale, "session.trace.stage.approvalDenied");
    case "approval_resolved":
      return tr(locale, "session.trace.stage.approvalResolved");
    case "turn_limit_reached":
      return tr(locale, "session.trace.stage.turnLimitReached");
    default:
      return tr(locale, "session.trace.stage.other");
  }
}
...
function toRiskLabel(locale: TraceLocale, riskLevel: string): string {
  ...
  if (normalized === "critical") {
    return tr(locale, "session.trace.risk.critical");
  }
  if (normalized === "high") {
    return tr(locale, "session.trace.risk.high");
  }
  if (normalized === "low") {
    return tr(locale, "session.trace.risk.low");
  }
  return "";
}
...
function formatIntentLabel(
  ...
  if (intentKind === "command" && value !== "") {
    return tr(locale, "session.trace.intent.command", { value });
  }
  if (intentKind === "path" && value !== "") {
    return tr(locale, "session.trace.intent.path", { value });
  }
  if (intentKind === "url" && value !== "") {
    return tr(locale, "session.trace.intent.url", { value });
  }
  if (intentKind === "query" && value !== "") {
    return tr(locale, "session.trace.intent.query", { value });
  }
...
function formatRunningPrimary(
  ...
  if (type === "model") {
    return tr(locale, "session.running.primary.modelAnalyzing");
  }
  if (type === "approval") {
    return tr(locale, "session.running.primary.approval", { tool: toolName });
  }
  if (type === "user_input") {
    return tr(locale, "session.running.primary.userInput");
  }
...
function composeSecondary(locale: TraceLocale, reasoning: string, operation: string): string {
  ...
  if (normalizedReasoning !== "" && normalizedOperation !== "") {
    return tr(locale, "session.running.secondary.reasoningOperation", {
      reasoning: normalizedReasoning,
      operation: normalizedOperation
    });
  }
  if (normalizedReasoning !== "") {
    return tr(locale, "session.running.secondary.reasoningOnly", {
      reasoning: normalizedReasoning
    });
  }
  if (normalizedOperation !== "") {
    return tr(locale, "session.running.secondary.operationOnly", {
      operation: normalizedOperation
    });
  }
