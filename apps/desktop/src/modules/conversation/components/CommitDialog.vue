<template>
  <div v-if="visible" class="commit-dialog-overlay" role="presentation" @click="onBackdropClick">
    <section class="commit-dialog" role="dialog" aria-modal="true" :aria-label="t('conversation.commitDialog.title')" @click.stop>
      <header class="commit-dialog-head">
        <h3>{{ t("conversation.commitDialog.title") }}</h3>
      </header>
      <label class="commit-dialog-label" for="commit-message">{{ t("conversation.commitDialog.messageLabel") }}</label>
      <textarea
        id="commit-message"
        ref="textareaRef"
      ...
      <div class="commit-dialog-actions">
        <button type="button" class="commit-dialog-btn" @click="$emit('close')">
          {{ t("conversation.commitDialog.cancel") }}
        </button>
        <button
          type="button"
          ...
          @click="onConfirm"
        >
          {{ pending ? t("conversation.commitDialog.confirming") : t("conversation.commitDialog.confirm") }}
        </button>
      </div>
    </section>
  </div>
