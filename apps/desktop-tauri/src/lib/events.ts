...
function summaryFrom(event: EventEnvelope): string {
  if (event.type === "tool_call") {
    return `Tool call: ${String(event.payload.tool_name ?? "unknown")}`;
  }
  if (event.type === "tool_result") {
    const ok = event.payload.ok === true ? "ok" : "error";
    return `Tool result: ${String(event.payload.call_id ?? "unknown")} (${ok})`;
  }
  if (event.type === "patch") {
  ...
  if (event.type === "done") {
    return `Run ${String(event.payload.status ?? "finished")}`;
  }
  if (event.type === "plan") {
    return String(event.payload.summary ?? "Plan updated");
  }
  return event.type;
}
