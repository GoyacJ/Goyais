...
func NewService(repo Repository, idempotencyTTL time.Duration, allowPrivateToPublic bool, logger *log.Logger) *Service {
	...
	return &Service{repo: repo, idempotencyTTL: idempotencyTTL, allowPrivateToPublic: allowPrivateToPublic, logger: logger}
}
...
func (s *Service) Submit(ctx context.Context, reqCtx RequestContext, commandType string, payload json.RawMessage, idempotencyKey, requestedVisibility string) (Command, error) {
	...
	created, err := s.repo.Create(ctx, CreateInput{Context: reqCtx, CommandType: commandType, Payload: payload, Visibility: visibility, IdempotencyKey: idempotencyKey, RequestHash: requestHash, Now: now, TTL: s.idempotencyTTL})
	if err != nil {
		return Command{}, err
	}
	if created.Reused {
	...
	result := map[string]any{"handled": true, "commandType": commandType, "executedAt": time.Now().UTC().Format(time.RFC3339Nano)}
...
func (s *Service) CreateShare(ctx context.Context, reqCtx RequestContext, resourceType, resourceID, subjectType, subjectID string, permissions []string, expiresAt *time.Time) (Share, error) {
	resourceType = strings.ToLower(strings.TrimSpace(resourceType))
	subjectType = strings.ToLower(strings.TrimSpace(subjectType))
	resourceID = strings.TrimSpace(resourceID)
	subjectID = strings.TrimSpace(subjectID)

	if (resourceType != "command" && resourceType != "asset") || resourceID == "" || subjectType != "user" || subjectID == "" {
	...
	case "asset":
		assetTarget, err := s.repo.GetAssetForShare(ctx, reqCtx, resourceID)
		if err != nil {
			if errors.Is(err, ErrNotImplemented) {
				return Share{}, err
			}
		...
		if reqCtx.TenantID != assetTarget.TenantID || reqCtx.WorkspaceID != assetTarget.WorkspaceID {
			return Share{}, &ForbiddenError{Reason: "workspace_mismatch"}
		}
		if reqCtx.UserID != assetTarget.OwnerID {
			has, err := s.repo.HasAssetPermission(ctx, reqCtx, assetTarget.ID, PermissionShare, time.Now().UTC())
			if err != nil {
				return Share{}, err
			}
			if !has {
				return Share{}, &ForbiddenError{Reason: "permission_denied"}
			}
		}
	}
	...
	return s.repo.CreateShare(ctx, ShareCreateInput{Context: reqCtx, ResourceType: resourceType, ResourceID: resourceID, SubjectType: subjectType, SubjectID: subjectID, Permissions: normalizedPermissions, ExpiresAt: expiresAt, Now: time.Now().UTC()})
}
...
func normalizePermissions(raw []string) ([]string, error) {
	...
	allowed := map[string]struct{}{PermissionRead: {}, PermissionWrite: {}, PermissionExecute: {}, PermissionManage: {}, PermissionShare: {}}
...
func validateCreateRequest(commandType string, payload json.RawMessage) error {
	if strings.TrimSpace(commandType) == "" || len(payload) == 0 {
		return ErrInvalidCommandRequest
	}
