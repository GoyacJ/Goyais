...
func (r *SQLiteRepository) Create(ctx context.Context, in CreateInput) (CreateResult, error) {
	...
	if in.IdempotencyKey == "" {
		...
		return CreateResult{Command: cmd}, nil
	}
	...
	if params.Cursor != "" {
		...
		rows, err := r.db.QueryContext(ctx,
			`SELECT c.id, c.tenant_id, c.workspace_id, c.owner_id, c.visibility, c.acl_json, c.command_type, c.payload, c.status, c.result, c.error_code, c.message_key, c.accepted_at, c.finished_at, c.created_at, c.updated_at
			 ...
			 LIMIT ?`,
			params.Context.TenantID, params.Context.WorkspaceID,
			params.Context.OwnerID, params.Context.UserID, now,
			createdAt.Format(time.RFC3339Nano), createdAt.Format(time.RFC3339Nano), id,
			pageSize,
		)
		if err != nil {
...
func (r *SQLiteRepository) HasCommandPermission(ctx context.Context, req RequestContext, commandID, permission string, now time.Time) (bool, error) {
	...
	err := r.db.QueryRowContext(ctx,
		`SELECT 1
		 ...
		 LIMIT 1`,
		req.TenantID, req.WorkspaceID, commandID, req.UserID, now.UTC().Format(time.RFC3339Nano), strings.ToUpper(permission),
	).Scan(&marker)
	if errors.Is(err, sql.ErrNoRows) {
		return false, nil
	}
	if err != nil {
		return false, fmt.Errorf("query command acl permission: %w", err)
	}
	return true, nil
}

func (r *SQLiteRepository) GetAssetForShare(ctx context.Context, req RequestContext, assetID string) (ShareAsset, error) {
	row := r.db.QueryRowContext(ctx,
		`SELECT id, tenant_id, workspace_id, owner_id
		 FROM assets
		 WHERE id = ? AND tenant_id = ? AND workspace_id = ?`,
		assetID, req.TenantID, req.WorkspaceID,
	)
	var item ShareAsset
	if err := row.Scan(&item.ID, &item.TenantID, &item.WorkspaceID, &item.OwnerID); err != nil {
		if errors.Is(err, sql.ErrNoRows) {
			return ShareAsset{}, ErrAssetNotFound
		}
		return ShareAsset{}, fmt.Errorf("query asset share target: %w", err)
	}
	return item, nil
}

func (r *SQLiteRepository) HasAssetPermission(ctx context.Context, req RequestContext, assetID, permission string, now time.Time) (bool, error) {
	...
	err := r.db.QueryRowContext(ctx,
		`SELECT 1
		 ...
		 WHERE a.tenant_id = ?
		   ...
		   AND EXISTS (SELECT 1 FROM json_each(a.permissions) p WHERE p.value = ?)
		 LIMIT 1`,
		req.TenantID, req.WorkspaceID, assetID, req.UserID, now.UTC().Format(time.RFC3339Nano), strings.ToUpper(permission),
	).Scan(&marker)
	if errors.Is(err, sql.ErrNoRows) {
		return false, nil
	}
...
func (r *SQLiteRepository) CreateShare(ctx context.Context, in ShareCreateInput) (Share, error) {
	...
	_, err = r.db.ExecContext(ctx,
		`INSERT INTO acl_entries(id, tenant_id, workspace_id, resource_type, resource_id, subject_type, subject_id, permissions, expires_at, created_by, created_at)
		 VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
		shareID, in.Context.TenantID, in.Context.WorkspaceID, in.ResourceType, in.ResourceID, in.SubjectType, in.SubjectID,
		string(permissionsRaw), expiresAtValue, in.Context.UserID, now.Format(time.RFC3339Nano),
	)
	if err != nil {
...
func (r *SQLiteRepository) ListShares(ctx context.Context, params ShareListParams) (ShareListResult, error) {
	...
	if params.Cursor != "" {
		...
		return ShareListResult{Items: items, NextCursor: nextCursor, UsedCursor: true}, nil
	}
	...
	rows, err := r.db.QueryContext(ctx,
		`SELECT id, tenant_id, workspace_id, resource_type, resource_id, subject_type, subject_id, permissions, expires_at, created_by, created_at
		 ...
		 ORDER BY created_at DESC, id DESC
		 LIMIT ? OFFSET ?`,
		params.Context.TenantID, params.Context.WorkspaceID, pageSize, offset,
	)
	if err != nil {
	...
	return ShareListResult{Items: items, Total: total}, nil
}
...
func (r *SQLiteRepository) DeleteShare(ctx context.Context, req RequestContext, shareID string) error {
	result, err := r.db.ExecContext(ctx,
		...
		shareID, req.TenantID, req.WorkspaceID, req.UserID,
	)
	if err != nil {
...
func (r *SQLiteRepository) AppendCommandEvent(ctx context.Context, req RequestContext, commandID, eventType string, payload []byte) error {
	...
	_, err := r.db.ExecContext(ctx,
		`INSERT INTO command_events(id, command_id, event_type, payload, created_at)
		 VALUES (?, ?, ?, ?, ?)`,
		newID("cevt"), commandID, eventType, string(payload), time.Now().UTC().Format(time.RFC3339Nano),
	)
	if err != nil {
...
func (r *SQLiteRepository) AppendAuditEvent(ctx context.Context, req RequestContext, commandID, eventType, decision, reason string, payload []byte) error {
	...
	_, err := r.db.ExecContext(ctx,
		`INSERT INTO audit_events(id, tenant_id, workspace_id, user_id, trace_id, command_id, event_type, resource_type, resource_id, decision, reason, payload, created_at)
		 VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
		newID("aevt"), req.TenantID, req.WorkspaceID, req.UserID, newID("trace"), commandID,
		eventType, "command", commandID, decision, reason, string(payload), time.Now().UTC().Format(time.RFC3339Nano),
	)
	if err != nil {
...
func (r *SQLiteRepository) SetStatus(ctx context.Context, req RequestContext, commandID, status string, result []byte, errorCode, messageKey string, finishedAt *time.Time) (Command, error) {
	...
	_, err := r.db.ExecContext(ctx,
		...
		status, resultVal, errorCodeVal, messageKeyVal, finishedAtVal, updatedAt,
		commandID, req.TenantID, req.WorkspaceID, req.OwnerID,
	)
	if err != nil {
...
func (r *SQLiteRepository) lookupActiveIdempotency(ctx context.Context, conn *sql.Conn, in CreateInput, now time.Time) (bool, string, string, error) {
	...
	err := conn.QueryRowContext(ctx,
		...
		in.Context.TenantID, in.Context.WorkspaceID, in.Context.OwnerID, in.IdempotencyKey, now.Format(time.RFC3339Nano),
...
func (r *SQLiteRepository) upsertIdempotencyFromConn(ctx context.Context, conn *sql.Conn, in CreateInput, commandID string, expiresAt, now time.Time) error {
	_, err := conn.ExecContext(ctx,
		...
		in.Context.TenantID, in.Context.WorkspaceID, in.Context.OwnerID, in.IdempotencyKey,
		in.RequestHash, commandID,
...
func (r *SQLiteRepository) insertCommand(ctx context.Context, req RequestContext, commandType string, payload []byte, visibility string, now time.Time) (Command, error) {
	...
	_, err := r.db.ExecContext(ctx,
		`INSERT INTO commands(id, tenant_id, workspace_id, owner_id, visibility, acl_json, command_type, payload, status, accepted_at, created_at, updated_at)
		 VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
		cmdID, req.TenantID, req.WorkspaceID, req.OwnerID,
		visibility, "[]", commandType, string(payload), StatusAccepted,
...
func (r *SQLiteRepository) getByID(ctx context.Context, req RequestContext, id string) (Command, error) {
	row := r.db.QueryRowContext(ctx,
		`SELECT id, tenant_id, workspace_id, owner_id, visibility, acl_json, command_type, payload, status, result, error_code, message_key, accepted_at, finished_at, created_at, updated_at
		 FROM commands
		 WHERE id = ? AND tenant_id = ? AND workspace_id = ? AND owner_id = ?`,
		id, req.TenantID, req.WorkspaceID, req.OwnerID,
	)
	cmd, err := scanCommand(row)
	if errors.Is(err, sql.ErrNoRows) {
		return Command{}, ErrNotFound
...
func scanCommand(row rowScanner) (Command, error) {
	var (
		cmd                                                           Command
		aclRaw, payloadRaw, acceptedAtRaw, createdAtRaw, updatedAtRaw string
		resultRaw, errorCodeRaw, messageKeyRaw, finishedAtRaw         sql.NullString
	)
	if err := row.Scan(&cmd.ID, &cmd.TenantID, &cmd.WorkspaceID, &cmd.OwnerID, &cmd.Visibility, &aclRaw, &cmd.CommandType, &payloadRaw, &cmd.Status, &resultRaw, &errorCodeRaw, &messageKeyRaw, &acceptedAtRaw, &finishedAtRaw, &createdAtRaw, &updatedAtRaw); err != nil {
		return Command{}, err
	}
...
func scanShare(row rowScanner) (Share, error) {
	...
	if err := row.Scan(&item.ID, &item.TenantID, &item.WorkspaceID, &item.ResourceType, &item.ResourceID, &item.SubjectType, &item.SubjectID, &permissions, &expiresAtRaw, &item.CreatedBy, &createdAtRaw); err != nil {
		return Share{}, err
	}
