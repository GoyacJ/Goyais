...
// Parse parses one raw composer input as either command or prompt payload.
...
// ParsePrompt parses one raw composer input as prompt payload only.
...
// ExtractResourceMentions extracts and deduplicates structured resource mentions.
...
// StripResourceMentions removes non-file resource mentions from prompt text and
// replaces file mentions with their normalized path token.
