...
func TestConvertMultiPathPasteToMentionsSupportsWindowsLineEndings(t *testing.T) {
	workdir := t.TempDir()
	...
	converted, changed := ConvertMultiPathPasteToMentions("a.txt\r\nb.txt\r\n", workdir)
	if !changed {
		t.Fatal("expected conversion for CRLF input")
	}
	if converted != "@a.txt @b.txt" {
		t.Fatalf("unexpected converted output %q", converted)
	}
}
