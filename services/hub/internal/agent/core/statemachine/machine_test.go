...
func TestCanTransitionAndAllowedTransitions(t *testing.T) {
	if !CanTransition(RunStateQueued, RunStateRunning) {
		t.Fatal("queued -> running should be allowed")
	}
	if CanTransition(RunStateQueued, RunStateCompleted) {
		t.Fatal("queued -> completed should be rejected")
	}

	matrix := AllowedTransitions()
	if len(matrix) == 0 {
		t.Fatal("expected non-empty transition matrix")
	}
	if _, exists := matrix[RunStateRunning]; !exists {
		t.Fatal("expected running state in transition matrix")
	}
}
