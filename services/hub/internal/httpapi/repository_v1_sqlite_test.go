...
func TestAuthzStoreCreatesHubRuntimeV1SchemaVersion(t *testing.T) {
	...
	if err := store.db.QueryRow(
		...
		t.Fatalf("query runtime v1 schema version failed: %v", err)
	}
	if version != hubRuntimeV1SchemaVersion {
		t.Fatalf("expected runtime v1 schema version %q, got %q", hubRuntimeV1SchemaVersion, version)
	}
}

func TestSQLiteRuntimeV1RepositoriesReplaceAndPaginate(t *testing.T) {
	store, err := openAuthzStore(":memory:")
	if err != nil {
		t.Fatalf("open authz store failed: %v", err)
	}
	...
	repos := NewSQLiteRuntimeV1RepositorySet(store.db)
