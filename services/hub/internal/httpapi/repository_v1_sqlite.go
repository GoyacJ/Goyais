...
// NewSQLiteRuntimeV1RepositorySet returns sqlite-backed runtime v1 repositories.
func NewSQLiteRuntimeV1RepositorySet(db *sql.DB) RuntimeV1RepositorySet {
	return RuntimeV1RepositorySet{
...
func (r *sqliteRuntimeSessionRepository) ReplaceAll(ctx context.Context, items []RuntimeSessionRecord) error {
	...
	return withWriteTx(ctx, r.db, func(tx *sql.Tx) error {
		if _, err := tx.ExecContext(ctx, `DELETE FROM runtime_sessions_v1`); err != nil {
			return err
		}
		for _, item := range items {
			...
			if _, err := tx.ExecContext(
				ctx,
				`INSERT INTO runtime_sessions_v1(
...
func (r *sqliteRuntimeRunRepository) ReplaceAll(ctx context.Context, items []RuntimeRunRecord) error {
	...
	return withWriteTx(ctx, r.db, func(tx *sql.Tx) error {
		if _, err := tx.ExecContext(ctx, `DELETE FROM runtime_runs_v1`); err != nil {
			return err
		}
		for _, item := range items {
			if _, err := tx.ExecContext(
				ctx,
				`INSERT INTO runtime_runs_v1(
...
func (r *sqliteRuntimeRunEventRepository) ReplaceAll(ctx context.Context, items []RuntimeRunEventRecord) error {
	...
	return withWriteTx(ctx, r.db, func(tx *sql.Tx) error {
		if _, err := tx.ExecContext(ctx, `DELETE FROM runtime_run_events_v1`); err != nil {
			return err
		}
		for _, item := range items {
			...
			if _, err := tx.ExecContext(
				ctx,
				`INSERT INTO runtime_run_events_v1(
...
func (r *sqliteRuntimeRunTaskRepository) ReplaceAll(ctx context.Context, items []RuntimeRunTaskRecord) error {
	...
	return withWriteTx(ctx, r.db, func(tx *sql.Tx) error {
		if _, err := tx.ExecContext(ctx, `DELETE FROM runtime_run_tasks_v1`); err != nil {
			return err
		}
		for _, item := range items {
			...
			if _, err := tx.ExecContext(
				ctx,
				`INSERT INTO runtime_run_tasks_v1(
...
func (r *sqliteRuntimeChangeSetRepository) ReplaceAll(ctx context.Context, items []RuntimeChangeSetRecord) error {
	...
	return withWriteTx(ctx, r.db, func(tx *sql.Tx) error {
		if _, err := tx.ExecContext(ctx, `DELETE FROM runtime_change_sets_v1`); err != nil {
			return err
		}
		for _, item := range items {
			payload := item.Payload
			...
			if _, err := tx.ExecContext(
				ctx,
				`INSERT INTO runtime_change_sets_v1(
...
func (r *sqliteRuntimeHookRecordRepository) ReplaceAll(ctx context.Context, items []RuntimeHookRecord) error {
	...
	return withWriteTx(ctx, r.db, func(tx *sql.Tx) error {
		if _, err := tx.ExecContext(ctx, `DELETE FROM runtime_hook_records_v1`); err != nil {
			return err
		}
		for _, item := range items {
			...
			if _, err := tx.ExecContext(
				ctx,
				`INSERT INTO runtime_hook_records_v1(
