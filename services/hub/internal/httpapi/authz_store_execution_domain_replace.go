...
func (s *authzStore) replaceExecutionDomainSnapshot(snapshot executionDomainSnapshot) error {
	...
	for _, item := range snapshot.Executions {
		...
		if _, err = tx.Exec(
			`INSERT INTO executions(id, workspace_id, conversation_id, message_id, state, mode, model_id, mode_snapshot, model_snapshot_json, project_revision_snapshot, queue_index, trace_id, created_at, updated_at)
			 VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?)`,
			item.ID,
			item.WorkspaceID,
			item.ConversationID,
