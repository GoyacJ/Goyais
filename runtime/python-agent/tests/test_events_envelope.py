...
def _load_validator() -> Draft7Validator:
    ...
    schema_dir = root / "packages" / "protocol" / "schemas" / "v1"
    ...
    envelope = schemas["goyais.protocol.v1.event-envelope"]
...
def test_tool_call_payload_requires_call_id():
    invalid_event = {
        ...
        "type": "tool_call",
        "payload": {"tool_name": "read_file", "args": {}, "requires_confirmation": False},
    }
...
def test_plan_payload_requires_summary():
    invalid_event = {
        ...
        "ts": "2026-02-20T00:00:00Z",
        "type": "plan",
        "payload": {"steps": ["read", "patch"]},
    }
    _assert_invalid(invalid_event)


def test_error_payload_requires_message():
    invalid_event = {
        "protocol_version": "1.0.0",
        "event_id": "evt_3",
        "run_id": "run_1",
