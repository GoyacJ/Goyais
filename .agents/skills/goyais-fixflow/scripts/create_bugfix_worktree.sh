...
usage() {
  cat <<'EOF'
Usage:
  create_bugfix_worktree.sh --topic <topic> [options]

Options:
  --topic <value>          Bugfix topic (required)
...
log() {
  printf '[goyais-fixflow] %s\n' "$*"
}

die() {
  printf '[goyais-fixflow] ERROR: %s\n' "$*" >&2
  exit 1
}
...
BRANCH="codex/${THREAD_ID_SLUG}-${TOPIC_SLUG}"
...
printf 'next_steps:\n'
printf '  1) cd %s\n' "${WORKTREE_PATH}"
printf '  2) start bug reproduction and fix\n'
