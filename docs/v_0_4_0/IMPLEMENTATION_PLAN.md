...
5. i18n 双语补齐。
6. 无障碍（键盘导航、focus trap、对比度）检查。
7. 侧边进程管理与打包流程验证。
8. 设计一致性优化（推荐按 Pencil MCP 设计方法落地）。

### 验收标准

1. 12 条以上 PRD 验收场景全部通过（含回滚、导出、模型同步、项目配置）。
...
| 集成测试 | 2,4,5,7,8 | API + DB + 队列 + 回滚 + 审批 + 模型同步 |
| E2E | 5,6,7,9 | 主屏幕流程、设置/账号信息菜单语义、异常恢复 |
