...
**进度**: 已完成（2026-03-08，分支 `goya/v0.5.0-refactor`，提交 `4cad8f9`）

- [x] 创建 `v0.5.0-refactor` 分支
- [x] 设置数据库迁移工具（golang-migrate）
- [x] 建立 feature flag 系统（支持渐进式迁移）
- [x] 更新 CI/CD 流水线（增加数据库测试）
- [x] 编写迁移脚本（内存数据 → SQLite）
...
**完成说明**:
- 已新增 `services/hub/migrations/000001_stage0_foundation.*.sql` 与 `embed.go`，并接入 `golang-migrate`
- 已新增 `FEATURE_SQLITE_REPO`、`FEATURE_EVENT_BUS`、`FEATURE_CQRS`
- CI 已增加 migration smoke
- 已提供 `go run ./cmd/hub migrate` 与 `scripts/migrate-to-v050.sh`
...
**进度**: 已完成主体（2026-03-08，提交 `4cad8f9`）；可选项“内存缓存层”未实现

#### Week 1: 领域模型定义

- [x] 创建 `services/hub/internal/domain/` 包
- [x] 定义聚合根：`Workspace`, `Session`, `Run`
- [x] 定义值对象：`WorkspaceID`, `SessionID`, `RunState`
- [x] 定义 Repository 接口
- [x] 编写领域模型单元测试
...
- [x] 实现 SQLite Repository（`internal/infrastructure/sqlite/`）
- [x] **新增**: 实现 `RunRepository` 和 `RunEventRepository`
- [x] 实现事件总线（内存版本）
- [x] 数据库迁移脚本（创建表结构）
- [x] Repository 集成测试
- [x] **新增**: Agent Engine 持久化集成测试
- [x] 性能基准测试（对比内存 map）
...
**Agent Engine 改造**:
- [x] 重构 `runtime/loop/engine.go` 使用 Repository
- [x] 实现细粒度锁（per-session）
- [ ] 添加内存缓存层（可选）
- [x] Engine 重启恢复测试

**完成说明**:
- 已新增 `internal/infrastructure/eventbus/memory.go`
- 已在 `internal/infrastructure/sqlite/` 下补齐 `workspace_repo.go`、`session_repo.go`、`run_repo.go`、`run_event_repo.go`、`loop_store.go`、`migrator.go`
- `loop.Engine` 已支持持久化快照、重启恢复和 per-session 锁
- 已新增基准测试，对比结果示例：`memory_map 454.1 ns/op`，`sqlite_repo 268171 ns/op`
- `httpapi.NewAppState` 已可在 `FEATURE_SQLITE_REPO=true` 时接入新持久化路径

**验证记录**:
- `cd services/hub && go test ./...`
- `pnpm contracts:check`
- `cd services/hub && go test -run '^$' -bench BenchmarkSessionRepositorySave -benchtime=20x ./internal/infrastructure/sqlite`
- `HUB_DB_PATH=/tmp/... scripts/migrate-to-v050.sh`

---

### 阶段 2: 后端应用层重构 (3 周，已完成)

**目标**: CQRS 命令/查询分离 + Agent 接口完善 + 资源配置继承

#### Week 1: 命令层 + Agent CommandBus

- [x] 创建 `internal/application/commands/` 包
- [x] 实现命令：`CreateSession`, `SubmitMessage`, `ControlRun`（覆盖 `approve|deny|resume|answer|stop`）
- [x] **新增**: 实现 `CommandBus` 接口（`internal/agent/runtime/slash/`）
- [x] **新增**: 实现 `ToolExecutor` 接口（已落地于 `internal/agent/tools/executor/`）
- [x] 命令处理器单元测试
- [x] 集成到现有 HTTP handlers

#### Week 2: 资源配置继承机制（新增）

- [x] 实现 `ResourceConfigService`（资源继承与验证）
- [x] 实现资源版本控制（`resource_configs` 表）
- [x] 实现项目资源绑定白名单（`project_resource_bindings` 表）
- [x] 实现 Session 资源快照（`session_resource_snapshots` 表）
- [x] 实现资源删除时的级联处理与自动降级
- [x] 添加资源变更事件推送（SSE）
- [x] 单元测试与集成测试
...
└── resource_config_domain_adapter.go  # HTTP adapter -> domain service

services/hub/internal/infrastructure/sqlite/
├── resource_config_repo.go     # 资源配置 Repository
└── （复用现有 SQLite 建表入口，无新增独立 migration 文件）
```

#### Week 2-3: 查询层 + Agent 权限优化 + Checkpoint 实现

- [x] 创建 `internal/application/queries/` 包
- [x] 实现查询：`GetSessionDetail`, `ListSessions`, `GetRunEvents`
- [x] **新增**: 实现统一权限评估流程（`UnifiedPermissionGate`）
- [x] **新增**: 添加权限审计日志
- [x] **新增**: 实现 Checkpoint 服务（Git + 非 Git 项目）
- [x] **新增**: 实现 Rollback API
- [x] 查询优化（索引、投影）
- [x] 查询层测试

**Checkpoint 实现**:
```
services/hub/internal/application/queries/
├── session.go                  # 查询应用服务入口
└── session_read_model.go       # 读模型/投影聚合
...
services/hub/internal/httpapi/
└── checkpoint_application_service.go  # HTTP adapter -> domain service/runtime

services/hub/internal/infrastructure/sqlite/
├── checkpoint_repo.go          # Checkpoint Repository
└── （复用现有 `session_checkpoints` 表，无新增独立 migration 文件）
```
