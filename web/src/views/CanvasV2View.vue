<template>
  <section class="ui-page">
    <PageHeader :title="t('page.canvas.v2Title')" :subtitle="t('page.canvas.v2Subtitle')">
      <template #actions>
    ...
    <div class="grid gap-3 xl:grid-cols-12">
      ...
      <SectionCard class="xl:col-span-6" :title="t('page.canvas.v2BoardTitle')" :subtitle="selectedTemplateId ?? '-'">
        <div class="space-y-3">
          ...
          <div class="flex flex-wrap items-center gap-2 rounded-lg border border-ui-border bg-ui-panel px-3 py-2 text-xs text-ui-muted">
            <span>{{ t('page.canvas.v2History') }}</span>
            <Button variant="secondary" :disabled="!canUndo" @click="onUndo">{{ t('page.canvas.v2Undo') }}</Button>
            <Button variant="secondary" :disabled="!canRedo" @click="onRedo">{{ t('page.canvas.v2Redo') }}</Button>
            <span class="ml-auto">{{ t('page.canvas.v2SelectedNode') }}: {{ selectedNodeId ?? '-' }}</span>
          </div>
      ...
      <SectionCard class="xl:col-span-3" :title="t('page.canvas.v2InspectorTitle')" :subtitle="selectedNodeId ?? '-'">
        <div class="space-y-3">
          <div class="rounded-lg border border-ui-border bg-ui-panel p-3 text-xs text-ui-muted">
            <div>{{ t('page.canvas.v2ConnectionRule') }}</div>
            <div class="mt-1 text-ui-text">{{ t('page.canvas.v2ConnectionRuleHint') }}</div>
          ...
          >
            ...
            <div>{{ t('page.canvas.v2NodeType') }}: {{ selectedNodeData.nodeType }}</div>
            ...
            <Button variant="destructive" :disabled="isBusy" @click="onRemoveSelectedNode">
              {{ t('page.canvas.v2ActionRemoveNode') }}
            </Button>
          </div>

          <div
            v-if="selectedNodeRuntime"
            class="canvas-v2-node-runtime space-y-1 rounded-lg border border-ui-border bg-ui-panel p-3 text-xs text-ui-text"
          >
            <div class="font-medium">{{ t('page.canvas.v2RuntimeTitle') }}</div>
            ...
            <div>{{ t('page.canvas.v2RuntimeArtifacts') }}: {{ selectedNodeRuntime.artifactCount }}</div>
            <div v-if="selectedNodeRuntime.logRef">{{ t('page.canvas.v2RuntimeLogRef') }}: {{ selectedNodeRuntime.logRef }}</div>
            <div v-if="selectedNodeRuntime.errorCode" class="text-ui-danger">
              {{ t('page.canvas.v2RuntimeErrorCode') }}: {{ selectedNodeRuntime.errorCode }}
            </div>
          </div>

          <div class="rounded-lg border border-ui-border bg-ui-panel p-3 text-xs">
            <div class="mb-2 font-medium text-ui-text">{{ t('page.canvas.v2PatchDiffTitle') }}</div>
            <div class="space-y-1 text-ui-muted">
              <div>{{ t('page.canvas.v2PatchAddedNodes') }}: {{ patchDiff.addedNodes.length }}</div>
              <div>{{ t('page.canvas.v2PatchRemovedNodes') }}: {{ patchDiff.removedNodes.length }}</div>
              <div>{{ t('page.canvas.v2PatchChangedNodes') }}: {{ patchDiff.changedNodes.length }}</div>
              <div>{{ t('page.canvas.v2PatchAddedEdges') }}: {{ patchDiff.addedEdges.length }}</div>
              <div>{{ t('page.canvas.v2PatchRemovedEdges') }}: {{ patchDiff.removedEdges.length }}</div>
            </div>
          </div>
          ...
          <div class="rounded-lg border border-ui-border bg-ui-panel p-3">
            ...
            <div class="space-y-1 text-xs text-ui-muted">
              <button
                ...
                :key="step.id"
                type="button"
                class="canvas-v2-step-item w-full rounded border px-2 py-1 text-left transition"
                :class="
              ...
              >
                ...
                <div class="mt-1 text-[11px]">
                  ...
                  <span>{{ t('page.canvas.v2RuntimeArtifacts') }}: {{ countArtifacts(step.artifacts) }}</span>
                </div>
              </button>
...
function onConnect(connection: Connection): void {
  ...
  if (!isTypeCompatible(sourceType, targetType)) {
    connectionError.value = t('page.canvas.v2ConnectionMismatch', { sourceType, targetType })
    pushToast({
      tone: 'error',
